...
module.exports = (data) => {
  console.log('Job processing now >> Queue type used: ', data.qType);
  /*
  ...
  const samples = data.length ? data : data.upsertData;
  const user = data.user;
  const reqStartTime = data.reqStartTime;
  const readOnlyFields = data.readOnlyFields;
  const errors = [];
  if (featureToggles.isFeatureEnabled('instrumentKue')) {
    const msg =
      `[KJI] Entered bulkUpsertSamples.js: job.id=blah ` +
  ...
  let dbEndTime;
  let successCount = 0;
  return cacheSampleModel.bulkUpsertByName(samples, user, readOnlyFields)
    .then((results) => {
      dbEndTime = Date.now();
      /*
